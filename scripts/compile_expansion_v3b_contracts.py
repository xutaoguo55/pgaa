#!/usr/bin/env python3
"""Compile result-blind unit contracts for expansion-v3b rescue platforms."""
...
def main() -> int:
    queue = load_frozen_v3b_candidate_queue(ROOT).set_index("candidate_id")
    platform_rows: list[dict[str, object]] = []
    unit_frames: list[pd.DataFrame] = []
    for spec in frozen_v3b_rescue_specs():
        base = {
        ...
        try:
            contract, split_map, _ = build_platform_unit_contract(
                ...
                split_seed="pgaa-expansion-v3b-contract-v1",
            )
    ...
    platform_path = evidence / "expansion_v3b_platform_contract_status.tsv"
    unit_path = evidence / "expansion_v3b_selected_unit_contract.tsv"
    platform.to_csv(platform_path, sep="\t", index=False)
    units.to_csv(unit_path, sep="\t", index=False)
    ...
    lines = [
        "# Expansion v3b Rescue Platform Contracts",
        "",
        ...
        *[f"- {row.candidate_id}: {int(row.n_selected_units)} selected units" for row in ready.itertuples()],
        "",
        "Candidates below the frozen selected-unit gate or still source-pending remain visible in the status TSV.",
    ]
    (ROOT / "docs/EXPANSION_V3B_CONTRACT_STATUS.md").write_text(
        "\n".join(lines) + "\n", encoding="utf-8"
    )
    print(f"Ready rescue platforms: {len(ready)}; selected units: {len(units)}")
    return 0
