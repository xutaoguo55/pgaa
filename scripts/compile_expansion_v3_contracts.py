#!/usr/bin/env python3
"""Compile result-blind unit contracts for expansion-v3 platforms."""
...
def main() -> int:
    queue = load_frozen_v3_candidate_queue(ROOT).set_index("candidate_id")
    platform_rows: list[dict[str, object]] = []
    unit_frames: list[pd.DataFrame] = []
    for spec in frozen_v3_candidate_specs():
        base = {
        ...
        try:
            contract, split_map, _ = build_platform_unit_contract(
                ...
                split_seed="pgaa-expansion-v3-contract-v1",
            )
    ...
    platform.to_csv(evidence / "expansion_v3_platform_contract_status.tsv", sep="\t", index=False)
    units.to_csv(evidence / "expansion_v3_selected_unit_contract.tsv", sep="\t", index=False)

    ready = platform[platform["contract_status"].eq("ready_for_scoring")]
    primary_ready = ready[ready["queue_role"].eq("primary_priority")]
    lines = [
        "# Expansion v3 Platform Contracts",
        "",
        "This is a metadata-only compilation. No expression-derived method outcome was used.",
        "",
        f"- Candidate platforms registered: {len(platform)}",
        f"- Primary candidates registered: {int(platform['queue_role'].eq('primary_priority').sum())}",
        f"- Platforms ready for scoring: {len(ready)}",
        f"- Primary platforms ready for scoring: {len(primary_ready)}",
        f"- Frozen selected units across all candidates: {len(units)}",
        ...
        "",
        "The v3 source-download gate passed, but the frozen metadata contract does not yet yield ten ready platforms.",
    ]
    (ROOT / "docs/EXPANSION_V3_CONTRACT_STATUS.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(
        f"Ready platforms: {len(ready)}; primary ready: {len(primary_ready)}; selected units: {len(units)}"
    )
    return 0
