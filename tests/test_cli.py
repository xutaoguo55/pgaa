...
def test_cli_csv_runner_writes_s1_s2_outputs(tmp_path):
    ...
    # S2 is a histogram-shape ranking diagnostic, not a small-sample discovery
    # test. Use a clearly separated synthetic response so this CLI smoke test
    # checks plumbing and ranking direction without overstating S2 stability.
    n_perturbed = 200
    n_control = 400
    ...
    X[:half, bimodal_col] -= 2.5
    X[half:n_perturbed, bimodal_col] += 2.5
