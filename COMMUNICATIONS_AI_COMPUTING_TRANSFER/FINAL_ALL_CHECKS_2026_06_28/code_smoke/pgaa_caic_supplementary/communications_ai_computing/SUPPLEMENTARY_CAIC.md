...
\textbf{Supplementary Figure 1.} Adamson 2016 BHLHE40 perturbation details. a, Gene-level PGAA-W Wasserstein versus PGAA-H histogram-shape diagnostic scores with UPR markers highlighted. b, Control and BHLHE40-perturbed expression distributions for representative UPR genes.
\end{center}
...
\textbf{Supplementary Figure 2.} ELANE heterogeneous expression pattern in Norman 2019 CEBPE CRISPRa. a, ELANE expression distribution across control and CEBPE-perturbed cells using 20 histogram bins. b, Histogram profiles used for the PGAA-H histogram-shape diagnostic comparison.
\end{center}
...
\textbf{Supplementary Figure 6.} PGAA distribution-aware Perturb-seq ranking workflow. The schematic summarizes the input data, preprocessing, PGAA-W Wasserstein statistic, PGAA-H histogram-shape diagnostic, permutation calibration, and outputs.
\end{center}
...
The main text summarizes the Norman multi-perturbation extension. This table gives the full curated target-panel benchmark used to support PGAA-W Wasserstein as the default ranking statistic. The benchmark uses processed Norman h5ad input, 2,000 HVGs plus forced inclusion of target-panel genes, and non-targeting controls sampled at up to a 3:1 control-to-perturbed ratio.
...
The five observational datasets are used for marker recovery rather than causal or clinical inference. The top-100 enrichment compares known marker recovery against background ranking.

| Dataset | Statistic | Top-100 enrichment | AUROC | Key known targets hit |
|---|---|---:|---:|---|
| CLL 20k | PGAA-W Wasserstein | 4.0x | 0.87 | CD79A, CD79B, MS4A1 |
| Sepsis 20k | PGAA-W Wasserstein | 2.1x | 0.87 | TCR pathway |
| RA 10k | PGAA-W Wasserstein | 2.5x | 0.87 | PBMC/monocyte-associated cytokine pathway |
| PBMC 3k | PGAA-W Wasserstein | 2.9x | 0.87 | Multi-lineage markers |
| IBD 10k | PGAA-W Wasserstein | 5.8x | 0.92 | Gut epithelial disease-associated markers |
