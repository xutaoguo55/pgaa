...
We present **PGAA**, a distribution-aware method and software implementation for heterogeneous single-cell response prioritization. PGAA uses two complementary statistics sharing a common calibration infrastructure: the PGAA-W quantile-grid Wasserstein approximation captures one-dimensional distributional shifts, and the S2 histogram-shape statistic (PGAA-H) is an exploratory diagnostic for responder-associated expression changes. The mathematical target is to rank non-target genes by perturbation-associated distributional discrepancy after coarse cell-state and library-size adjustment. The contribution is not a new distance metric, a new statistical test, or a new mathematical theorem—it is a principled integration of existing tools (1D Wasserstein, histogram peak analysis, permutation calibration) into a transparent, guardrailed method with explicit documentation of failure modes. PGAA is not a general-purpose computational framework; it is a specific ranking component designed to complement existing Perturb-seq tools. Additional modules for conditional MI and Fisher NB scoring are provided as software extensions. All statistics are accompanied by Storey upper-tail calibration diagnostics that let users assess reliability before interpreting results. Figure 1a illustrates the response regimes, and Figure 1b summarizes the workflow.

**Contribution.** PGAA is not a new distance metric, a new statistical test, or a new mathematical theorem. It is a principled integration of existing distributional tools (1D Wasserstein, histogram peak analysis, permutation calibration) into a transparent, guardrailed ranking method for heterogeneous perturbation responses—a computational decision-support component, not a general-purpose framework. The method has three layers: (1) two complementary statistics sharing common permutation calibration infrastructure; (2) explicit guardrails (Storey upper-tail diagnostic, pilot n_bins sweep, negative-control requirement for PGAA-H) that define when a ranking can be trusted and when it cannot; and (3) a reproducible Python/R implementation with a command-line interface, source-data rebuild scripts, and a locked evidence-source registry. The contribution is the integration, the guardrails, and the documentation of failure modes—not the individual statistical components. Systematic comparison against other distributional tests (two-sample KS, energy distance, Cramér-von Mises) is not performed in this submission and is a key limitation.
...
PGAA is designed as a modular ranking method for heterogeneous single-cell responses. The Wasserstein statistic is the recommended starting score for full-distribution shifts, whereas the S2 histogram-shape statistic (PGAA-H) is an exploratory diagnostic for responder-associated expression-shape changes. The framework first adjusts expression for coarse cell-state and library-size effects, then ranks genes by distributional discrepancy between perturbed and control cells. Within-cluster permutation is used to provide empirical ranking evidence, and the Storey upper-tail diagnostic is used as a guardrail for calibration rather than as a guarantee of genome-wide discovery.
