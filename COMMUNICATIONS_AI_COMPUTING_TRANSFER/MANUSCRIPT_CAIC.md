...
**Contribution.** PGAA addresses a general computational problem—distribution-aware ranking of heterogeneous single-cell perturbation responses—through three components: (1) two complementary statistics (PGAA-W for full-distribution shifts, PGAA-H for histogram-shape changes) with a shared within-cluster permutation calibration layer; (2) a calibration guardrail protocol (Storey upper-tail diagnostic and histogram-bin sensitivity for PGAA-H) that defines when the ranking can be trusted; and (3) a reproducible Python/R implementation with a command-line interface, source-data rebuild scripts, and a locked evidence-source registry. The contribution is not a new mathematical theorem or a universal discovery claim; it is a transparent, guardrailed ranking component designed to sit alongside existing Perturb-seq tools.
