...
def check_docx() -> list[Check]:
    ...
    checks.append(Check("docx_embedded_main_figures", "PASS" if len(media) >= 4 else "FAIL", f"{len(media)} embedded media files"))
    return checks
...
def check_figures_tables() -> list[Check]:
    ...
    checks.append(Check("main_figure_sequence", "PASS" if main_figures == [1, 2, 3, 4] else "FAIL", str(main_figures)))
