#!/usr/bin/env python3
"""Generate a cleaner Norman 2019 CEBPE figure for the CM main text."""
...
ROOT = CM_ROOT.parent
OUT = CM_ROOT / "figures_png" / "figure_norman_main_cm.png"
...
def main() -> None:
    ...
    ax = axes[0]
    ax.scatter(s2.loc[~is_known, "S2"], y_s2[~is_known], s=7, color="#9E9E9E", alpha=0.35, linewidth=0)
    ax.scatter(
        s2.loc[is_known, "S2"], y_s2[is_known],
    ...
    for gene in ["ELANE", "PRTN3"]:
        ...
        ax.annotate(
            gene,
            (row["S2"], -np.log10(float(row["p_value_perm"]) + 1e-300)),
            xytext=(5, 5),
